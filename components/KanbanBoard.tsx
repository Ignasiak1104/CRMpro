    ...
    <div className="flex flex-col h-full space-y-6">
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <div className="flex-1 flex overflow-x-auto pb-4 gap-6 scrollbar-hide">
                ...
                <div className="p-5 flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <span className={`w-2 h-2 rounded-full ${
                      stage === 'Pozyskany' ? 'bg-green-500' : 
                      stage === 'Utracony' ? 'bg-rose-500' : 'bg-indigo-500'
                    }`} />
                ...
                <Droppable droppableId={stage}>
                  {(provided, snapshot) => (
                    <div 
                      ...
                      className={`flex-1 overflow-y-auto px-4 pb-4 space-y-4 transition-all duration-300 rounded-b-[32px] ${
                          ...
                          {(provided, snapshot) => (
                            <div 
                              ...
                              className={`bg-white p-5 rounded-2xl border transition-all duration-300 ${
                                snapshot.isDragging 
                                  ? 'shadow-2xl border-indigo-500 scale-105 z-50 rotate-1' 
                                  : 'shadow-sm border-slate-200/60 hover:border-indigo-300 hover:shadow-md'
                              }`}
                            >
                              <div className="flex justify-between items-start mb-3">
                                <p className="text-[9px] text-indigo-600 font-black uppercase tracking-widest bg-indigo-50 px-2 py-0.5 rounded-full">
                                  {getCompanyName(deal.companyId)}
                                </p>
                                <span className="text-slate-300 text-[10px]">⋮⋮</span>
                              </div>
                              
                              <h4 className="font-bold text-slate-900 text-sm leading-tight mb-4 min-h-[40px]">
                                {deal.title}
                              </h4>
                              ...
                              <div className="flex justify-between items-end border-t border-slate-50 pt-3">
                                <div>
                                  ...
                                  <p className="text-slate-900 font-black text-sm">
                                    {deal.value.toLocaleString()} <span className="text-[10px] text-slate-400">PLN</span>
                                  </p>
                                </div>
                                
                                <div className="flex -space-x-2">
                                  <div className="w-8 h-8 rounded-full bg-slate-100 border-2 border-white flex items-center justify-center text-[10px] font-bold text-slate-500 shadow-sm overflow-hidden">
          ...
          {/* Pusty element na końcu dla lepszego marginesu przy przewijaniu */}
