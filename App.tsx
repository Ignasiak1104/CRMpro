...
const App: React.FC = () => {
  ...
  // Form States
  const [newCompany, setNewCompany] = useState({ name: '', industry: '', status: 'Prospect' as Company['status'], website: '' });
  const [newContact, setNewContact] = useState({ firstName: '', lastName: '', email: '', phone: '', role: '', companyId: '' });
  ...
  const handleSaveCompany = async (e: React.FormEvent) => {
    ...
    setNewCompany({ name: '', industry: '', status: 'Prospect', website: '' });
    setSyncing(false);
  };
  ...
  const handleAddContact = async (e: React.FormEvent) => {
    ...
    setActiveModal(null);
    setNewContact({ firstName: '', lastName: '', email: '', phone: '', role: '', companyId: '' });
    setSyncing(false);
  };
  ...
  const handleAddDeal = async (e: React.FormEvent) => {
    ...
    // Fixed: Removed accidental insertion of Deal data into the 'contacts' table
  ...
  const handleReorderStage = (pipelineId: string, stageIndex: number, direction: 'up' | 'down') => {
    setPipelines(prev => prev.map(p => {
      ...
      // Fix: Corrected undefined variable 'index' to 'stageIndex' and removed redundant swap logic
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <main className="flex-1 ml-64 p-10 max-w-7xl mx-auto">
        {!isSupabaseConfigured && (
          <div className="mb-6 bg-indigo-600 text-white p-4 rounded-2xl flex justify-between items-center shadow-lg shadow-indigo-100">
            <p className="text-sm font-bold">Tryb Demo: Dane lokalne (Dodaj klucze Supabase w .env, aby włączyć chmurę)</p>
          </div>
        )}
        ...
        {currentView === 'companies' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-4 flex-1 max-w-xl mr-6">
                <div className="relative flex-1">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">🔍</span>
                  <input 
                    type="text" 
                    placeholder="Szukaj firmy po nazwie lub branży..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full bg-white border border-slate-100 rounded-2xl py-3 pl-12 pr-4 text-sm font-medium focus:ring-2 focus:ring-indigo-600 outline-none shadow-sm transition-all"
                  />
                  {searchQuery && (
                    <button 
              ...
              <button onClick={() => { setEditingCompanyId(null); setNewCompany({ name: '', industry: '', status: 'Prospect', website: '' }); setActiveModal('company'); }} className="bg-indigo-600 text-white px-5 py-2.5 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 whitespace-nowrap">+ Dodaj firmę</button>
            </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-50 text-sm">
                  {filteredCompanies.length > 0 ? filteredCompanies.map(c => (
                    ...
                    <tr>
                      <td colSpan={4} className="px-8 py-10 text-center text-slate-400 italic">Brak wyników wyszukiwania.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-4 flex-1 max-w-xl mr-6">
                <div className="relative flex-1">
                  ...
                  <input 
                    type="text" 
                    placeholder="Szukaj kontaktu po nazwisku lub e-mailu..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full bg-white border border-slate-100 rounded-2xl py-3 pl-12 pr-4 text-sm font-medium focus:ring-2 focus:ring-indigo-600 outline-none shadow-sm transition-all"
                  />
              ...
              <button onClick={() => setActiveModal('contact')} className="bg-indigo-600 text-white px-5 py-2.5 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 whitespace-nowrap">+ Dodaj kontakt</button>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-50 text-sm">
                  {filteredContacts.length > 0 ? filteredContacts.map(co => (
      ...
      {selectedCompany && (
        <>
          <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-[2px] z-40 transition-opacity animate-in fade-in" onClick={() => setSelectedCompanyId(null)} />
          <CompanyDetailPanel 
            ...
            onAddTask={() => {
              setNewTask(prev => ({ ...prev, relatedId: selectedCompanyId }));
              setActiveModal('task');
            }}
          />
        </>
          ...
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b border-slate-50">
              <h3 className="text-2xl font-black text-slate-900 tracking-tight">
                {activeModal === 'company' && (editingCompanyId ? 'Edytuj Firmę' : 'Nowa Firma')}
                {activeModal === 'contact' && 'Nowy Kontakt'}
                {activeModal === 'deal' && 'Nowy Szansa'}
                {activeModal === 'task' && 'Nowe Zadanie'}
              </h3>
            </div>
            ...
            <form onSubmit={
              ...
              {activeModal === 'contact' && (
                <>
                  ...
                  <input type="text" placeholder="Telefon" value={newContact.phone} onChange={e => setNewContact({...newContact, phone: e.target.value})} className="w-full bg-slate-50 border-none rounded-2xl p-4 text-sm font-bold focus:ring-2 focus:ring-indigo-600 outline-none" />
              ...
              {activeModal === 'task' && (
                <>
                  ...
                  <select required value={newTask.relatedId} onChange={e => setNewTask({...newTask, relatedId: e.target.value})} className="w-full bg-slate-50 border-none rounded-2xl p-4 text-sm font-bold focus:ring-2 focus:ring-indigo-600 outline-none">
                    <option value="">Powiąż z firmą...</option>
                    {companies.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                  </select>
                </>
              )}
